...
import valueVMTypeOptionsQuery from './query/valueVMTypeOptions';

const debug = require('debug')('app:component:valueVMTypeOptions');

const dirname = `${__dirname}/logs/valueVMTypeOptions`;
    ...
    .map(async entityType =>
      test(entityType, async () => {
        ...
        for (let i = 0; i < count; i++) {
          ...
          debug('path', path);
          const result = await graphql(valueVMTypeOptionsQuery, { path });
          await writeVM(entityType, i, 'valueVMTypeOptions', result);
