...
import Organization from '../../entities/entityData/Organization';
import { getGUID } from '../service';

const debug = require('debug')('app:component:base:entities:Organization');
...
const Root = ({ location, location: { pathname } }) => {
  ...
  return (
    <QueryRenderer
      ...
      query={graphql`
        query OrganizationQuery($path: String!) {
          entity(path: $path) {
            ...Organization_entity
          }
          identity(path: $path) {
          ...
          valueVMOptions {
            ...Organization_valueVMOptions
          }
          valueVMTypeOptions(path: $path) {
            ...Organization_valueVMTypeOptions
          }
        }
      `}
      variables={{ path }}
